...
trait StringFieldBehaviours extends FieldBehaviours {
  ...
  def fieldWithMaxLength(
    ...
    s"not bind strings longer than $maxLength characters" in
      forAll(stringGenerator -> "longString") { string =>
        val result = form.bind(Map(fieldName -> string)).apply(fieldName)
        result.errors mustEqual Seq(lengthError)
      }

  def fieldStartingWithCapitalLetter(form: Form[_], fieldName: String, requiredError: FormError): Unit =
  ...
  def telephoneNumberField(form: Form[_], fieldName: String, invalidError: FormError): Unit =
    ...
    "not bind UTRs that have been used for other business trustees" in {
      ...
      forAll(utrGenerator, intGenerator) { (utr, size) =>
        val updatedUserAnswers =
          (0 until size).foldLeft(emptyUserAnswers)((acc, i) => acc.set(torg.UtrPage(i), utr).success.value)
