...
class LeadTrusteeOrganisationPrintHelper @Inject()(answerRowConverter: AnswerRowConverter, countryOptions: CountryOptions) {
  ...
  private def answers(userAnswers: UserAnswers, name: String, index: Int, draftId: String)
    ...
    Seq(
      ...
      bound.yesNoQuestion(nonTaxable.CountryOfResidenceInTheUkYesNoPage(index), s"$prefix.nonTaxable.countryOfResidenceInTheUkYesNo", ntRts.CountryOfResidenceInTheUkYesNoController.onPageLoad(index, draftId).url),
      bound.countryQuestion(nonTaxable.CountryOfResidenceInTheUkYesNoPage(index), nonTaxable.CountryOfResidencePage(index), s"$prefix.nonTaxable.countryOfResidence", ntRts.CountryOfResidenceController.onPageLoad(index, draftId).url),
