  ...
  private def answers(userAnswers: UserAnswers, name: String, index: Int, draftId: String)(implicit
    ...
    Seq(
      ...
      bound.nameQuestion(
        ...
        s"$prefix.name",
        rts.NameController.onPageLoad(index, draftId).url,
        canEdit = !isLeadTrusteeMatched
      ),
      bound.dateQuestion(
        DateOfBirthPage(index),
        s"$prefix.dateOfBirth",
        rts.DateOfBirthController.onPageLoad(index, draftId).url,
        canEdit = !isLeadTrusteeMatched
      ),
      bound.yesNoQuestion(
      ...
      bound.yesNoQuestion(
        TrusteeNinoYesNoPage(index),
        s"$prefix.ninoYesNo",
        rts.NinoYesNoController.onPageLoad(index, draftId).url,
        canEdit = !isLeadTrusteeMatched
      ),
      bound.ninoQuestion(
        TrusteesNinoPage(index),
        s"$prefix.nino",
        rts.NinoController.onPageLoad(index, draftId).url,
        canEdit = !isLeadTrusteeMatched
      ),
      bound.enumQuestion(
      ...
      bound.passportDetailsQuestion(
        ...
        rts.IDCardDetailsController.onPageLoad(index, draftId).url
      ),
      bound.passportDetailsQuestion(
        PassportDetailsPage(index),
        s"$prefix.passportDetails",
        rts.PassportDetailsController.onPageLoad(index, draftId).url
      ),
      bound.yesNoQuestion(
      ...
      bound.yesNoQuestion(
        ...
        rts.LiveInTheUKYesNoController.onPageLoad(index, draftId).url
      ),
