...
class TrusteeIndividualPrintHelper @Inject()(answerRowConverter: AnswerRowConverter) {
  ...
  private def answers(userAnswers: UserAnswers, name: String, index: Int, draftId: String)
    ...
    Seq(
      ...
      bound.nameQuestion(NamePage(index), s"$prefix.name", Some(NameController.onPageLoad(index, draftId).url)),
      ...
      bound.dateQuestion(DateOfBirthPage(index), s"$prefix.dateOfBirth", Some(DateOfBirthController.onPageLoad(index, draftId).url)),
      ...
      bound.ninoQuestion(NinoPage(index), s"$prefix.nino", Some(NinoController.onPageLoad(index, draftId).url)),
