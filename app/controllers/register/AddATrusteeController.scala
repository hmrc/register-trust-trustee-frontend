...
class AddATrusteeController @Inject() (
  ...
  def submitLead(draftId: String): Action[AnyContent] = actions(draftId).async { implicit request =>
    val index: Int = MAX - 1
      ...
      .fold(
        ...
        value =>
          request.userAnswers
            ...
            .flatMap(_.set(TrusteeOrLeadTrusteePage(index), LeadTrustee)) match {
            case Success(updatedAnswers) =>
              registrationsRepository.set(updatedAnswers).map { _ =>
                setTaskStatus(updatedAnswers, draftId, value)
                if (value == YesNow) {
  ...
  private def setTaskStatus(userAnswers: UserAnswers, draftId: String, selection: AddATrustee)(implicit
    hc: HeaderCarrier
  ): Future[HttpResponse] = {
