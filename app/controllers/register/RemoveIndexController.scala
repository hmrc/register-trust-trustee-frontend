...
class RemoveIndexController @Inject() (
  ...
  private def redirect(draftId: String): Result = Redirect(
    controllers.register.routes.AddATrusteeController.onPageLoad(draftId)
  )
    ...
    form
      ...
      .fold(
        (formWithErrors: Form[_]) =>
          Future.successful(
            BadRequest(view(formWithErrors, draftId, index, name(request.trustee), prefix(request.trustee)))
          ),
        remove =>
          if (remove) {
