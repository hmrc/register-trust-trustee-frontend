...
class IDCardDetailsYesNoController @Inject()(
  ...
  private val form = yesNoFormProvider.withPrefix("trusteeIDCardDetailsYesNo")
  ...
  def onPageLoad(index: Int, draftId: String): Action[AnyContent] = actions(index, draftId) {
    implicit request =>
      ...
      Ok(view(preparedForm, draftId, index, name.toString))
  }
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(view(formWithErrors, draftId, index, name.toString))),
