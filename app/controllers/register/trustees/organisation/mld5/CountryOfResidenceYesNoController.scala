...
class CountryOfResidenceYesNoController @Inject() (
  ...
  private val form: Form[Boolean] = formProvider.withPrefix("trustee.organisation.5mld.countryOfResidenceYesNo")

  def onPageLoad(index: Int, draftId: String): Action[AnyContent] =
    actions.identifiedUserWithData(draftId).andThen(requireName(index)) { implicit request =>
  ...
  def onSubmit(index: Int, draftId: String): Action[AnyContent] =
    actions.identifiedUserWithData(draftId).andThen(requireName(index)).async { implicit request =>
      form
        .bindFromRequest()
        .fold(
